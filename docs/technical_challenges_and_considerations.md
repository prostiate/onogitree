...
Building a multi-repository Git client involves several critical concurrency, operating system, and data consistency challenges. This document explains each concern in detail along with the architectural solutions and mitigations in **OnoGitTree**.
...
- **Go Goroutine Worker Pool with Semaphore Channels**:
  - Limit concurrent network operations to **4–6 parallel workers**.
...
flowchart TD
    ...
    JobChan --> Pool{"Worker Pool (Max 5 Goroutines)"}
...
### The Concern:
With 20+ open repositories, expanding branches, tags, stashes, and commit histories can quickly generate **3,000+ DOM nodes**, causing scroll lag and memory bloat.

### Solution & Mitigation:
- **Virtualized Tree Rendering (`@tanstack/solid-virtual`)**:
  - Flattens the hierarchical tree into a computed visible items array.
  - Only the 25–40 items currently visible on screen are rendered into the real DOM.
