...
import { RepoStatus, FileStatus, CommitSummary, CommitFileChange } from '../../types/git';
...
export const RepoDashboard: Component<RepoDashboardProps> = (props) => {
  const [contextMenuPos, setContextMenuPos] = createSignal<{ x: number; y: number; commit: CommitSummary } | null>(null);
  ...
  const getFileChangeBadge = (status: CommitFileChange['status']) => {
    switch (status) {
      case 'added':
        return <span class="px-1.5 py-0.5 bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 rounded text-[9.5px] font-mono font-bold">ADDED</span>;
      case 'deleted':
        return <span class="px-1.5 py-0.5 bg-rose-500/15 border border-rose-500/30 text-rose-400 rounded text-[9.5px] font-mono font-bold">DELETED</span>;
      case 'renamed':
        return <span class="px-1.5 py-0.5 bg-cyan-500/15 border border-cyan-500/30 text-cyan-300 rounded text-[9.5px] font-mono font-bold">RENAMED</span>;
      default:
        return <span class="px-1.5 py-0.5 bg-amber-500/15 border border-amber-500/30 text-amber-300 rounded text-[9.5px] font-mono font-bold">MODIFIED</span>;
    }
  };
    ...
    <div class="flex-1 flex flex-col overflow-y-auto p-6 space-y-6 text-gray-200 select-none">
      ...
      <div class="bg-[#11141D] border border-gray-800 rounded-2xl p-6 space-y-4 shadow-xl">
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs font-mono text-gray-500">Right-click or click a commit to inspect changed files</span>
        </div>
                              ...
                              <div class="space-y-3 pt-2">
                                ...
                                <div class="bg-[#0A0C13] p-3 rounded-lg border border-gray-800 space-y-2 text-xs">
                                  <div class="flex items-center justify-between text-gray-400 font-mono text-[11px]">
                                    <span class="truncate">Commit SHA: {detail().hash}</span>
                                    <button
                                      onClick={() => copyText(detail().hash, `sha-${detail().hash}`)}
                                      class="px-2 py-0.5 bg-gray-800 hover:bg-gray-700 text-gray-200 rounded flex items-center gap-1 cursor-pointer"
                                    >
                                      <Show when={copiedHash() === `sha-${detail().hash}`} fallback={<Copy class="w-3 h-3" />}>
                                        <Check class="w-3 h-3 text-emerald-400" />
                                      </Show>
                                      <span>{copiedHash() === `sha-${detail().hash}` ? 'Copied' : 'Copy SHA'}</span>
                                    </button>
                                  </div>
                                  ...
                                  <div class="flex items-center gap-3 text-[11px] font-mono font-bold pt-1">
                                    <span class="text-gray-400">{detail().files.length} changed files</span>
                                    <span class="text-emerald-400">+{detail().totalAdditions} additions</span>
                                    <span class="text-rose-400">-{detail().totalDeletions} deletions</span>
                                  </div>
                                </div>
                                ...
                                <div class="space-y-1">
                                  ...
                                  <div class="divide-y divide-gray-800/60 border border-gray-800 rounded-lg overflow-hidden bg-[#0A0C13]">
                                    <For each={detail().files}>
                                      {(file) => (
                                        <div
                                          onClick={() => repoStore.selectFileForDiff(file.path, false, detail().hash)}
